...
pub struct PostSerializable<AccountId, BlockNumber> {
    pub id: PostId,
...
impl<T: Trait> From<Post<T>> for PostSerializable<T::AccountId, T::BlockNumber> {
...
impl<T: Trait> Module<T> {
    pub fn get_posts_by_ids(post_ids: Vec<PostId>) -> Vec<PostSerializable<T::AccountId, T::BlockNumber>> {
    ...
    fn get_posts_slice<F: FnMut(&Post<T>) -> bool>(
        ...
        limit: u64,
        mut compare_fn: F,
    ) -> Vec<PostSerializable<T::AccountId, T::BlockNumber>> {
        let mut posts = Vec::new();
    ...
    /*pub fn get_post_replies(post_id: PostId) -> Vec<PostSerializable<T::AccountId, T::BlockNumber>> {
